...
#>
    ...
    if (!(Test-IsAdmin)){
       ...
       $Script = Invoke-Command -ScriptBlock {wscript.exe $Filepath} -ArgumentList $Filepath
       Register-ScheduledJob -Name BootService -ScriptBlock $Script -Trigger $Trig -ScheduledJobOption $Option -RunNow
